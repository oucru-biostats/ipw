ipwplot <- function(weights, timevar = NULL, binwidth = NULL, logscale = TRUE, xlab = NULL, ylab = NULL, main = "", ref = TRUE, ...){
	if (!is.null(timevar)){
		...
		if (is.null(xlab)) xlab <- deparse(match.call()$timevar)
		if (is.null(ylab) & logscale == FALSE) ylab <- deparse(match.call()$weights)
		if (is.null(ylab) & logscale == TRUE) ylab <- paste("log(", deparse(match.call()$weights), ")", sep = "")
		if(logscale == TRUE){
	...
	if (is.null(timevar)){
		if (is.null(xlab) & logscale == FALSE) xlab <- deparse(match.call()$weights)
		if (is.null(xlab) & logscale == TRUE) xlab <- paste("log(", deparse(match.call()$weights), ")", sep = "")
